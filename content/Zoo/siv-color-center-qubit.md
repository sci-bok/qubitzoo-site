...
- [[purcell-protection-via-detuning]] — the >100× cavity Purcell enhancement that makes SiV a fast spin-photon interface uses the same $\kappa(g/\Delta)^2$ framework as circuit-QED Purcell protection, but the optical ZPL transition operates in the enhancement (resonant) rather than the suppression (detuned) regime.
